...
describe('withZodSchema', () => {
  test('valid state: no errors thrown', async () => {
    ...
    const addon = withZodSchema(schema);
  ...
  test('invalid state: throws errors on submit', async () => {
  ...
  test('works via createForm', async () => {
    const form = createForm({ email: 'bad', age: 10 }, withZodSchema(schema));
  ...
  test('fields not in visible list are skipped', async () => {
    ...
    const addon = withZodSchema(schema);
    addon._setup!(api as any);
    // Only pass 'age' — email errors should be absent
